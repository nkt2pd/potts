...
int update_site(Properties main_properties, int k, double beta, Site *spin, int Ns, int L);
double MC_sweep(Properties main_properties, double beta, Site *spin, int Ns, int L);
void Monte_Carlo_Sim(Measurements main_measurements, Properties main_properties, double beta, Site *spin, int Ns, int L, const string L_name);
...
int main(int argc, char *argv[]) {
    ...
    for(double T = 1.5; T>0; T -= del_T) {
        ...
        Monte_Carlo_Sim(main_measurements, main_properties, 1./T, spin, Ns, L, L_name);
...
int update_site(Properties main_properties, int k, double beta, Site *spin, int Ns, int L) {
    ...
    for(int l=0; l<N_nn1; l++) {
        delS += (main_properties.V_clock[p_new][spin[k].nn1[l]->potts] - main_properties.V_clock[spin[k].potts][spin[k].nn1[l]->potts]);
    }
...
double MC_sweep(Properties main_properties, double beta, Site *spin, int Ns, int L) {
    int hits = 0;
    for(int i=0; i<Ns; i++) {
        hits += update_site(main_properties, i, beta, spin, Ns, L);
    }
...
void Monte_Carlo_Sim(Measurements main_measurements, Properties main_properties, double beta, Site *spin, int Ns, int L, const string L_name) {
    ...
    for (int i = 0; i < thermalize; i++) {
        accepted += MC_sweep(main_properties, beta, spin, Ns, L);
    }
    ...
    for (int n = 0; n < ndata; n++) {
        ...
        double e = main_measurements.clock_energy(main_properties, spin, Ns, L);

        E1 = (n * E1 + e) / (n + 1.);
        E2 = (n * E2 + e*e) / (n + 1.);

        double mag = main_measurements.potts_magnetization(spin, Ns, L);
